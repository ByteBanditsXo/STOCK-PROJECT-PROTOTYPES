...
# OpenAI API key
openai.api_key = 'sk-proj-tyMIADs4Denm5q3TWGxpT3BlbkFJ5fR73W06AP8nfwnbkEZC'
...
def calculate_rsi(ticker):
    ...
    data = stock.history(period='1y')['Close']
    delta = data.diff()
    gain = delta.where(delta > 0, 0.astype(delta.dtype)).astype(float)
...
def get_news():
    ...
    for article in articles:
        analysis = openai.ChatCompletion.create(
            ...
            messages=[
                {"role": "system", "content": "Analyze the sentiment of this news article and provide a recommendation (buy, sell, hold):"},
                {"role": "user", "content": f"{article['title']} {article['description']}"}
            ],
            max_tokens=50
        )
        article['sentiment'] = analysis.choices[0].message['content'].strip()
...
def extract_ticker_symbol_or_name(tokens):
    for token in tokens:
        if token.upper() in company_to_ticker.values():
            return token.upper()
        elif token.lower() in company_to_ticker.keys():
            return company_to_ticker[token.lower()]
    return None
